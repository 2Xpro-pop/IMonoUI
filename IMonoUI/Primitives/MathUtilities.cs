...
internal static class MathUtilities
{
    ...
    public static MonoPoint GetEllipsePoint(MonoPoint centre, double radiusX, double radiusY, double angle)
    {
        return new MonoPoint(radiusX * Math.Cos(angle) + centre.X, radiusY * Math.Sin(angle) + centre.Y);
    }
    ...
    internal static int WhichPolygonSideIntersects(
        uint cPoly,
        ReadOnlySpan<MonoVector> pPtPoly,
        MonoVector ptCurrent,
        MonoVector vecEdge)
    {
        uint nPositive = 0;
        uint nNegative = 0;
        ...
        var vecEdgeNormal = new MonoPoint(-vecEdge.Y, vecEdge.X);

        for (var i = 0; i < cPoly; i++)
        {
            var vecCurrent = ptCurrent - pPtPoly[i];
            var rDot = MonoVector.Dot(vecCurrent, vecEdgeNormal);
    ...
    internal static bool DoPolygonsIntersect(
        uint cPolyA,
        ReadOnlySpan<MonoVector> pPtPolyA,
        uint cPolyB,
        ReadOnlySpan<MonoVector> pPtPolyB)
    {
