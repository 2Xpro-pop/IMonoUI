...
#pragma warning restore CA1859 // Use concrete types when possible for improved performance
    ...
    static KnownColors()
    {
        ...
        foreach (var field in Enum.GetValues(typeof(KnownColor))
            )
        {
            var knownColor = (KnownColor)field;
            if (knownColor == KnownColor.None) continue;

            knownColorNames.Add(field.ToString()!, knownColor);

            // some known colors have the same value, so use the first
            if (!knownColors.ContainsKey((uint)knownColor))
            {
                knownColors.Add((uint)knownColor, field.ToString()!);
            }
        }
    ...
    public static MonoColor ToColor(this KnownColor color)
    {
        return MonoColor.FromUInt32((uint)color);
    }
}
